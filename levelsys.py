...
bot_channel = [1266037717859303484]
talk_channels = [1266037682799116308]
...
class levelsys(commands.Cog):
    ...
    async def on_ready(self):
        dict = {}
        guild = self.client.get_guild(1042059295849722007)
            ...
            if channel.type is discord.ChannelType.text:
                ...
                messages = [
                    message async for message in channel.history(
                        ...
                        after=datetime.today().replace(day=1))
                ]
                for x in messages:
        ...
        print("it's done hobo")

    @commands.Cog.listener()
    async def on_message(self, message):
        if message.author.id in self.leveldata:
            self.leveldata[
                message.author.id] = self.leveldata[message.author.id] + 1
        else:
    ...
    async def rank(self, ctx):
        ...
        if ctx.author.id in self.leveldata:
            ...
            embed = discord.Embed(title="{}'s level stats".format(
                ctx.author.name),
                                  description="",
                                  color=0x397882)
            ...
            embed.add_field(name="Rank",
                            value=f"{rank+1}/{ctx.guild.member_count}",
                            inline=True)
